...
export class CartService {
  ...
  addProduct(product: Product, quantity = 1): Observable<Cart> {
    return this.http.post<CartResponse>(this.apiUrl, { productId: product._id, quantity }).pipe(
      map(response => response.cart),
      tap(cart => this.cart.set(cart))
    );
  }
  ...
  updateQuantity(productId: string, quantity: number): Observable<Cart> {
    return this.http.put<CartResponse>(`${this.apiUrl}/${productId}`, { quantity }).pipe(
      map(response => response.cart),
      tap(cart => this.cart.set(cart))
    );
  }
  ...
  clear(): Observable<void> {
    return this.http.delete<{ status: string; message: string }>(this.apiUrl).pipe(
      map(() => undefined),
      tap(() => this.cart.set({ items: [] }))
    );
  }
}
